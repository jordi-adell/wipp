...
namespace test {
    ...
    TEST(wippTest, wippCopyTest) {
        ...
        DEBUG_STREAM(
                " min: " << fmin << " max: " << fmax << " : " << 2 * (fmin / 3) + fmax / 3 << " " << 2 * fmin / 3);
        ...
        short inSbuffer[len] = {smin, static_cast<short>(2 * (smin / 3) + smax / 3),
                                static_cast<short>(smin / 3 + 2 * (smax / 3)), smax};
        int inIbuffer[len] = {smin, static_cast<int> (2 * (imin / 3) + imax / 3),
                              static_cast<int> (imin / 3 + 2 * (imax / 3)), imax};
        unsigned short inUSbuffer[len] = {usmin, static_cast<unsigned short int> (2 * (usmin / 3) + usmax / 3),
                                          static_cast<unsigned short int> (usmin / 3 + 2 * (usmax / 3)), usmax};
        ...
        for (int i = 0; i < len; ++i) {
            TRACE_STREAM("D: " << inDbuffer[i] << " D: " << oDbuffer[i]);
            EXPECT_EQ(inDbuffer[i], oDbuffer[i]);
            TRACE_STREAM("F: " << inFbuffer[i] << " F: " << oFbuffer[i]);
            EXPECT_EQ(inFbuffer[i], oFbuffer[i]);
            TRACE_STREAM("S: " << inSbuffer[i] << " S: " << oSbuffer[i]);
            EXPECT_EQ(inSbuffer[i], oSbuffer[i]);
            TRACE_STREAM("I: " << inIbuffer[i] << " I: " << oIbuffer[i]);
            EXPECT_EQ(inIbuffer[i], oIbuffer[i]);
            TRACE_STREAM("US: " << inIbuffer[i] << " US: " << oIbuffer[i]);
    ...
    TEST(randTest, vaules) {
        size_t length = 1000;
        wipp::wipp_rand_t *rand;
        wipp::init_rand_gaussian(&rand, 0, 0);
        double frame[length];
        wipp::rand(rand, frame, length);
        for (size_t i = 0; i < length; ++i) {
            TRACE_STREAM(frame[i])
            EXPECT_FALSE(std::isnan(frame[i]));
        }
    }
    ...
    TEST(sqrTest, sqrt_sqr) {
        ...
        for (size_t i = 0; i < length; ++i)
            EXPECT_DOUBLE_EQ(sqrt_frame[i] * sqrt_frame[i], frame[i]);
        for (size_t i = 0; i < length; ++i)
            EXPECT_DOUBLE_EQ(sqr_frame[i], frame[i] * frame[i]);
    ...
    TEST(div, complex) {
        ...
        for (int i = 0; i < length; ++i) {
            EXPECT_DOUBLE_EQ(ref[i].re, c[i].re);
            EXPECT_DOUBLE_EQ(ref[i].im, c[i].im);
        }
...
//      frame[i] = i*1000 - 500;
        ...
        for (size_t i = 0; i < length; ++i) {
            ...
            if (frame[i] >= 0)
                EXPECT_GE(out_frame[i], frame[i]);
            else
                EXPECT_DOUBLE_EQ(out_frame[i], -1000);
        }
        ...
        for (size_t i = 0; i < length; ++i) {
            EXPECT_THAT(out_frame[i], AnyOf(Gt(0), Eq(-1000)));
//                EXPECT_TRUE(out_frame[i] > 0 || out_frame[i] == -1000);
            if (frame[i] > 0)
        ...
        for (size_t i = 0; i < length; ++i) {
            ...
            if (frame[i] < 0)
                ...
                EXPECT_DOUBLE_EQ(out_frame[i], 20);
        }
    ...
    TEST(iirTest, filter) {
        ...
        // Need to change definition of a, and be compared to default implementation
        double a_coefs[n] = {1, 0.97};
        double b_coefs[n] = {1, 0};

        double signal[length] = {1000, 1000, 1000, 1000, 0, 0, 0, 0, 0, 0};
        double reference[length] = {1.0000e3, 0.0300e3, 0.9709e3, 0.05822699e3, -0.05648019e3, 0.0547857e3,
                                    -0.0531422e3, 0.0515479e3, -0.05000150061e3, 0.04850146e3};
        double filtered[length];

        wipp::init_iir(&iir, a_coefs, n, b_coefs, n);

        ASSERT_FALSE(iir == nullptr);
    ...
    TEST(firTest, filter) {
        ...
        for (int n_coefs = 64; n_coefs < 512; n_coefs += 64) {
            ...
            for (double freq = 0.01; freq < 0.49; freq += 0.005) {
                ...
                if (freq < lf * 0.9)
                    EXPECT_LT(gain, -15) << "Filtered Power " << freq << ": " << gain << " dB";
                else if (lf * 1.1 < freq && freq < hf * 0.9)
                    EXPECT_NEAR(0, gain, 5) << "Filtered Power " << freq << ": " << gain << " dB";
                else if (freq > hf * 1.1)
                    EXPECT_LT(gain, -15) << "Filtered Power " << freq << ": " << gain << " dB";
    ...
    TEST(firTest, sinc) {
        ...
        for (int i = 0; i < length / 2; ++i) {
            ...
            TRACE_STREAM("i: " << i << ", f: " << f << ", FFT abs: " << magnitude[i]);
            if (f < fmin)EXPECT_LT(magnitude[i], 1);
            else if (f > 1.3 * fmin && f < fmax * 0.99)
                EXPECT_NEAR(1, magnitude[i], 0.2);
            else if (f > fmax)
...
//  save_buffer(frame, length/2, "datat");
        ...
        for (double fmin = 0.2; fmin < 0.5; fmin = fmin + delta_bw) {
            for (double fmax = fmin + delta; fmax < 0.5; fmax = fmax + delta_bw) {
                ...
                for (double freq = 0; freq < 0.5; freq = freq + delta) {
                    wipp::tone(signal, length, sinusoidalAmplitud, freq, phase);
                    ...
                    if (fpower < 0.001)
                        fpower = 0;
                    TRACE_STREAM(
                            "BW: " << std::setprecision(2) << fmin << "-" << fmax << "; F: " << freq << ", SP: "
                                   << spower << ", FP: " << fpower);

                    if (fmax - fmin > 0.2) {
                        if (freq < fmin)
                            EXPECT_LE(fpower, sinusoidalPower / 4);
                        else if (freq < (fmax + fmin) * 0.985 / 2)
                            EXPECT_GE(fpower, prevp);
                        else if (freq < (fmax + fmin) * 1.015 / 2)
                            EXPECT_NEAR(fpower, prevp, sinusoidalPower / 6);
                        else if (freq <= 1.02 * fmax)
                            EXPECT_LE(fpower, 1.015 * prevp);
                        else
                            EXPECT_LE(fpower, sinusoidalPower / 4);
                    }
    ...
    TEST(circularBufferTest, write_read) {
        ...
        for (data_position = 0; data_position < 2 * cf_length && free > 0;) {
            ...
            TRACE_STREAM("Free: " << free << " stored: " << stored);
    ...
    TEST(fftTest, init_delete) {
        ...
        for (int i = 1; i < 512; ++i) {
            if (i % 100 == 0) TRACE_STREAM("up to order: " << i);
            wipp::wipp_fft_t *fft;
            wipp::init_fft(&fft, i);
    ...
    void fftTestTransform(double freq, int fft_length, const std::vector<double> &fft_ref) {
        ...
        for (size_t i = 0; i < fft_length; ++i)
            EXPECT_NEAR(signal_fft[i], fft_ref[i], 1e-4) << "Sample number: " << i;
        ...
        for (size_t i = 0; i < fft_length; ++i)
            EXPECT_NEAR(signal[i], isignal[i], 1e-12);
        wipp::delete_fft(&fft);
    }
    ...
    TEST(fftTest, transform) {
        ...
        SCOPED_TRACE("Testing sinusoidal signal FFT with frequency 0.08");
        fftTestTransform(0.08, fft_length, fft_ref);

        fft_ref = {-0.0000, 0.0000, 0.0000, -0.0000, 0.0000, 0.0000, -0.0000, -0.0000, 0.0000, -0.0000, 0.0000,
                   0.0000,
                   ...
                   0.0000,
                   0.0000, 0.0000, -0.0000, 0.0000, 0.0000, 0.0000,
                   0.0000, 0.0000, -0.0000, -128.00, 0.0000, -0.0000, 0.0000, -0.0000, -0.0000, -0.0000, 0.0000,
                   -0.0000, 0.0000, -0.0000, 0.0000, -0.0000, -0.0000, 0.0000,
                   0.0000, -0.0000, 0.0000, 0.0000, 0.0000, -0.0000, 0.0000, -0.0000, 0.0000, 0.0000, -0.0000,
                   0.0000,
                   0.0000, -0.0000, 0.0000, -0.0000, -0.0000, -0.0000,
        ...
        SCOPED_TRACE("Testing sinusoidal signal FFT with frequency 0.25.");
    ...
    TEST(testWindowing, hanning) {
        ...
        double hanning16[16] = {0, 0.0432, 0.1654, 0.3455, 0.5523, 0.7500, 0.9045, 0.9891, 0.9891, 0.9045, 0.7500,
                                0.5523, 0.3455, 0.1654, 0.0432, 0};
        double hanning32[32] = {0, 0.0102, 0.0405, 0.0896, 0.1555, 0.2355, 0.3263, 0.4243, 0.5253, 0.6253, 0.7202,
                                0.8061, 0.8794, 0.9372, 0.9771, 0.9974, 0.9974, 0.9771, 0.9372, 0.8794, 0.8061,
                                0.7202,
                                0.6253, 0.5253, 0.4243, 0.3263, 0.2355, 0.1555, 0.0896, 0.0405, 0.0102, 0};

        for (size_t n = 16; n <= 2048; n = n * 2) {
            TRACE_STREAM("Hanning window length " << n);
            double frame[n];
            wipp::set(1, frame, n);
            ...
            for (size_t i = 0; i < n; ++i) {
                if (n == 16)
                    ...
                    EXPECT_NEAR((1 - cos(2 * M_PI * i / (n - 1))) / 2, frame[i], 1e-12);
            }
    ...
    TEST(testWindowing, ola) {
        ...
        for (size_t window_size = 1024; window_size < 4096; window_size = window_size + 64) {
            ...
            TRACE_STREAM("Window size: " << window_size << ", ola length:" << ola_length << ", window_shift: "
                                         << window_shift);
    ...
    TEST(testMedian, median) {
        ...
        double dbuffer[] = {0.1, 0.5, 0.3, 0.8, 0.2, 0.9, 0.4, 0.2, 0.7, 0.009, 0.7, 0.5, 0.3, 0.2, 10.5, 0.4, 0.3,
                            0.8,
                            0.7, 0.9, 0.7, 0.9, 0.5, 0.1, 0.5, 60.7, 90.8, 0.7, 0.5, 0.4, 0.009, 0.7, 0.5, 0.4, 0.3,
                            0.1, 0.6, 0.8, 0};
    ...
    TEST(testMedian, median_filter) {
        ...
        int reference[] = {1, 1, 1, 2, 3, 5, 5, 6, 6, 8, 9, 9, 9, 9, 8, 7, 5, 4, 3, 3};
        int obuffer[length];

        median_filter(buffer, obuffer, length, 5);
        for (size_t i = 0; i < length; ++i) {
            EXPECT_EQ(obuffer[i], reference[i]) << "i: " << i;
        }
    ...
    TEST(signal, tone) {
        ...
        tone(i_buffer, length, 10000*magnitude, frequency, phase);

        for (int i = 0; i < length; ++i) {
            double value = magnitude * cos(2 * M_PI * frequency * static_cast<double>(i) + phase);
            EXPECT_NEAR(value, d_buffer[i], 1e-5);
            EXPECT_NEAR(static_cast<float>(value), f_buffer[i], 1e-4);
            EXPECT_NEAR(static_cast<int>(10000*value), i_buffer[i], 1);
        }
...
//                            *   *
//                              *
//    |________-period-_________________|_______________________
//
///   |_|___4___|___1___|___2___|____3___|___4___| --> zones


    int getSampleZone(size_t samplePosition, int periodInSamples, size_t phaseDelayInSamples, double assymetry) {
        ...
        double zoneThreeEnd = round((1.0f*3/4 + assymetryInPercentage*1/4) * periodInSamples);

        if (baseSample <= zoneOneEnd) return 1;
        if (baseSample <= zoneTwoEnd) return 2;
        if (baseSample <= zoneThreeEnd) return 3;
        else return 4;
    }
        ...
        if (prevZone == zone) {
            switch (zone) {
                ...
                case 3:
                    ...
                    break;
                default:
                    throw(std::runtime_error("Unknown triangle Zone"));
            }
        }
        prevZone = zone;
    }

    size_t convertPhaseInRadiansToDelaySamples(double phaseInRadians, int periodInSamples) {
        while (phaseInRadians < 0) phaseInRadians += 2 * M_PI;
        while (phaseInRadians >= 2 * M_PI) phaseInRadians -= 2 * M_PI;
        return static_cast<size_t>(llround((phaseInRadians / (2 * M_PI)) * periodInSamples));
    }

    void test_evalTriangle(const double *buffer, size_t bufferLength,
                           int periodInSamples, double phaseInRadians, double assymetry, double offset) {
    ...
    void test_symetricNoOffsetTriangle(size_t bufferLength, double trianglePeriodInPercentage,
                                       double trianglePhaseInRadians) {
        int periodInSamples = static_cast<int>((trianglePeriodInPercentage * bufferLength) / 100);
        double buffer[bufferLength];
        wipp::triangle(buffer, bufferLength, periodInSamples, trianglePhaseInRadians, 0, 0);
        test_evalTriangle(buffer, bufferLength, periodInSamples, trianglePhaseInRadians, 0, 0);
    }

    void test_asymetricNoOffsetTriangle(size_t bufferLength, double trianglePeriodInPercentage,
                                        double trianglePhaseInRadians, double assymetry) {
        int periodInSamples = static_cast<int>((trianglePeriodInPercentage * bufferLength) / 100);
        while(assymetry < -M_PI) assymetry += 2*M_PI;
        while(assymetry >  M_PI) assymetry -= 2*M_PI;
        double buffer[bufferLength];
        wipp::triangle(buffer, bufferLength, periodInSamples, trianglePhaseInRadians, assymetry, 0);
        test_evalTriangle(buffer, bufferLength, periodInSamples, trianglePhaseInRadians, assymetry, 0);
    }

    void test_symetricOffsetTriangle(size_t bufferLength, double trianglePeriodInPercentage,
                                     double trianglePhaseInRadians, double offset) {
        int periodInSamples = static_cast<int>((trianglePeriodInPercentage * bufferLength) / 100);
        double buffer[bufferLength];
        wipp::triangle(buffer, bufferLength, periodInSamples, trianglePhaseInRadians, 0, offset);
        test_evalTriangle(buffer, bufferLength, periodInSamples, trianglePhaseInRadians, 0, offset);
    }

    TEST(testTriangle, triangle) {
        size_t bufferLength;
        double trianglePeriodInPercentage, trianglePhaseInRadians;
        ...
        for (bufferLength = 100; bufferLength < 1000; bufferLength += 100)
            for (trianglePeriodInPercentage = 50;
                ...
                for (trianglePhaseInRadians = 0;
                    ...
                    test_symetricNoOffsetTriangle(bufferLength, trianglePeriodInPercentage, trianglePhaseInRadians);

                    double edge = 0.2;
                    for (double assymetry = -M_PI*edge; assymetry <=  edge*M_PI; assymetry += M_PI_4                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                ) {
                        TRACE_STREAM("Assymetry: " << assymetry << "(" << assymetry*M_1_PI << ")");
                        test_asymetricNoOffsetTriangle(bufferLength, trianglePeriodInPercentage,
                                                       trianglePhaseInRadians, assymetry);
                    }
                    for (double offset = 1; offset < 100; offset += 25) {
                        TRACE_STREAM("Offset: " << offset);
                        test_symetricOffsetTriangle(bufferLength, trianglePeriodInPercentage,
                                                    trianglePhaseInRadians, offset);
                    }
